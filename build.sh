...
aapt2 compile --dir app/src/main/res -o "$RESZIP"
...
KEYSTORE_PASSWORD="${KEYSTORE_PASSWORD:-qqaiassist}"
KEY_ALIAS="${KEYSTORE_ALIAS:-qqaiassist}"
KEY_PASSWORD="${KEY_PASSWORD:-$KEYSTORE_PASSWORD}"

# 优先使用 CI Secret 里的签名，否则本地自动生成自签名 keystore
...
if [ ! -f "$KEYSTORE" ]; then
    keytool -genkeypair \
        ...
        -dname "CN=QQAiAssist,O=Local,C=CN"
fi
apksigner sign \
    --ks "$KEYSTORE" \
    --ks-pass "pass:$KEYSTORE_PASSWORD" \
    --key-pass "pass:$KEY_PASSWORD" \
    --out "$OUT/QQAiAssist.apk" \
    "$APKOUT/aligned.apk"

echo "Build OK: $OUT/QQAiAssist.apk"
